...
export default function TabletCard<T>({
  row,
  index,
  columns,
  selectable,
  ...
  const secondaryCols = columns.slice(4);
        ...
        <div className="flex items-start gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
          ...
          {visibleActions.length <= 3 ? (
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              />
              {showDropdown && (
                <div className="absolute right-0 top-9 z-[100] min-w-[200px] rounded-2xl shadow-2xl border border-border bg-card/95 backdrop-blur-xl p-1.5 overflow-hidden">
                  {visibleActions.map((action, ai) => {
                    const icon = typeof action.icon === "function" ? action.icon(row) : action.icon;
                    const lbl = typeof action.label === "function" ? action.label(row) : action.label;
                    const disabled = action.disabled ? action.disabled(row) : false;
