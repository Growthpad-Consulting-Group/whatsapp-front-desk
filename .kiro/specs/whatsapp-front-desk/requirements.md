...
| Term | Definition |
|---|---|
| Business | A tenant — a single service business using the platform |
| Owner | The staff member with the `owner` role who manages the business account |
| Customer | An end-user who books services via WhatsApp; has no platform account |
| Conversation session | A stateful WhatsApp exchange between a customer and the bot, scoped to one booking flow |
| Slot | A specific available time window for a service, computed from operating hours and existing appointments |
| Deposit | A partial upfront payment required to confirm a booking |
| Payment link | A URL generated by the payment provider (Paystack) that the customer uses to pay |
| Reminder rule | A configured trigger that sends a WhatsApp message at a defined time relative to an appointment or invoice event |
| Human handoff | The state where the bot stops responding and flags the conversation for manual owner intervention |
| RLS | Row-Level Security — Supabase/Postgres feature used to enforce per-tenant data isolation |
| WABA | WhatsApp Business Account — the Meta account linked to the business's WhatsApp number |
| E.164 | International phone number format, e.g. +2348012345678 |
